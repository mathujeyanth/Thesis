...
def plan_trajectory(move_group, destination_pose, start_joint_angles): 
    ...
    p = Pose()
    p.position = destination_pose.position
    p.orientation = destination_pose.orientation
    move_group.set_pose_target(destination_pose)
    #move_group.set_position_target([p.position.x,p.position.y,p.position.z])
...
def setup_scene():
    ...
    #p1 = PoseStamped()
    #p1.header.frame_id = move_group.get_planning_frame()
    #p1 = set_pose(p1, [0, -0.25, -0.02])
    #scene.add_box("box", p1, (0.1, 0.1, 0.18*2))
