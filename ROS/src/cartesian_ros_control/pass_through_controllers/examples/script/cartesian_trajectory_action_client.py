...
class Client(object):
    ...
    def test(self):
        ...
        def random_point():
            ...
            p.pose.orientation.x = 0#q[0]
            p.pose.orientation.y = 1#q[1]
            p.pose.orientation.z = 0.01#q[2]
            p.pose.orientation.w = 0.01#q[3]
            return p

        # Random 2-point trajectory
        duration = 10
        p1 = random_point()
        p1.pose.position.x = -0.13686
        p1.pose.position.y = -0.26718
        p1.pose.position.z = 0.1469
        p2 = random_point()
        p2.pose.position.x = -0.14
        p2.pose.position.y = -0.3
        p2.pose.position.z = 0.15
