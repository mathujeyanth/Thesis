	...
	void CreateNobFloor() {
		...
		for (int i = 0; i < amountInX; i++) {
			for (int j = 0; j < amountInZ; j++) {
				...
				Vector3 newPos = new Vector3(scaleFactor*0.04f + scaleFactor*0.08f * (float)(i - amountInX / 2), 0.008f*scaleFactor+0.5f*floorDims.y, scaleFactor*0.04f + scaleFactor*0.08f * (float)(j - amountInZ / 2));
